...
const Header = () => {
  ...
  useEffect(() => {
    const applyFlips = () => {
      if (isMobile()) return;
      const MARGIN = 16;
      document.querySelectorAll(".dropdown-submenu, .nested-submenu, .deep-submenu").forEach((el) => {
        const list = el.querySelector(":scope > .submenu-list, :scope > .nested-list, :scope > .deep-list");
        if (!list) return;
  ...
  const handleProductsClick = (e) => {
    ...
    setOpenSub(null); setOpenNested(null); setOpenDeep(null);
  };
  ...
  const handleSubClick = (name, path) => (e) => {
    if (isMobile()) {
      ...
      setOpenNested(null); setOpenDeep(null);
    } else if (path) {
      navigate(path);
      closeMenu();
    }
    ...
    <header className="site-header">
      ...
      <div className="top-bar">
        ...
        <div className="top-social">
          <a href="#fb" aria-label="Facebook"><i className="ti ti-brand-facebook" aria-hidden="true"></i></a>
          <a href="#linkedin" aria-label="LinkedIn"><i className="ti ti-brand-linkedin" aria-hidden="true"></i></a>
          <a href="#twitter" aria-label="Twitter / X"><i className="ti ti-brand-x" aria-hidden="true"></i></a>
          <a href="#instagram" aria-label="Instagram"><i className="ti ti-brand-instagram" aria-hidden="true"></i></a>
        </div>
      </div>
      ...
      <nav className="navbar">
        ...
        <ul className={`nav-links${menuOpen ? " show-menu" : ""}`}>
          ...
          <li className={`dropdown${openDropdown === "products" ? " mobile-open" : ""}`}>
            <button type="button" className="dropdown-toggle" onClick={handleProductsClick}
              aria-expanded={openDropdown === "products"} aria-haspopup="true">
            ...
            <ul className="dropdown-menu">
              ...
              <li className={`dropdown-submenu${openSub === "modular" ? " mobile-sub-open" : ""}`}>
                <button type="button" onClick={handleSubClick("modular")}>
                  ...
                  <i className="ti ti-chevron-right side-arrow" aria-hidden="true"></i>
                </button>
                <ul className="submenu-list">

                  {/* MS Containers → opens nested with Premium label + links */}
                  <li className={`nested-submenu${openNested === "ms-containers" ? " mobile-nested-open" : ""}`}>
                    <button type="button" onClick={handleNestedClick("ms-containers", "/ms-containers")}>
                      <span>MS Containers</span>
                      <i className="ti ti-chevron-right side-arrow" aria-hidden="true"></i>
                    </button>
                    <ul className="nested-list">
                      {/* Premium Containers = section heading, not clickable */}
                      <li><Link to="/premium-ms-containers" onClick={closeMenu} className="nested-section-heading" style={{cursor: 'pointer', textDecoration: 'none'}}>Premium Containers</Link></li>
                      <li><Link to="/executive-containers" onClick={closeMenu}>Executive Container</Link></li>
                      <li><Link to="/lr-value-containers" onClick={closeMenu}>L&amp;R Values</Link></li>
                    </ul>
                  </li>
                ...
                <ul className="submenu-list">
                  ...
                  <li className={`nested-submenu${openNested === "prefab" ? " mobile-nested-open" : ""}`}>
                    ...
                    <ul className="nested-list">
                      ...
                      <li><Link to="/prefab-mushroom" onClick={closeMenu}>Mushroom Plants</Link></li>
                      <li><Link to="/prefab-industrial-enclosures" onClick={closeMenu}>Industrial Enclosures</Link></li>
                      <li><Link to="/prefab-staff-canteens" onClick={closeMenu}>Prefabricated Staff Canteens</Link></li>
                      <li><Link to="/prefab-dry-wall" onClick={closeMenu}>Prefab Drywall</Link></li>
                      <li><Link to="/prefab-railway-shelters" onClick={closeMenu}>Prefab Railway &amp; Telecom Shelters</Link></li>
                    </ul>
                  </li>

                  {/* Sandwich Panels */}
                  <li className={`nested-submenu${openNested === "sandwich" ? " mobile-nested-open" : ""}`}>
                    <button type="button" onClick={handleNestedClick("sandwich", "/prefab-sandwich-panels")}>
                      <span>Sandwich Panels</span>
                      <i className="ti ti-chevron-right side-arrow" aria-hidden="true"></i>
                    </button>
                    <ul className="nested-list">
                      {/* PUF → Wall & Roof (4th level) */}
                      <li className={`deep-submenu${openDeep === "puf-panels" ? " mobile-deep-open" : ""}`}>
                        <button type="button" onClick={handleDeepClick("puf-panels", "/puf-sandwich-panels")}>
                          <span>PUF Sandwich Panels</span>
                          <i className="ti ti-chevron-right side-arrow" aria-hidden="true"></i>
                        </button>
                        <ul className="deep-list">
                          <li><Link to="/wall-puf-panels" onClick={closeMenu}>Wall PUF Panels</Link></li>
                          <li><Link to="/roof-puf-panels" onClick={closeMenu}>Roof PUF Panels</Link></li>
                        </ul>
                      </li>
                      ...
                      <li><Link to="/glasswool-panels" onClick={closeMenu}>Glasswool Panels</Link></li>
                    </ul>
                  </li>

                  {/* Rooftop Room */}
                  <li className={`nested-submenu${openNested === "rooftop" ? " mobile-nested-open" : ""}`}>
                    <button type="button" onClick={handleNestedClick("rooftop")}>
                      <span>Rooftop Room</span>
                      <i className="ti ti-chevron-right side-arrow" aria-hidden="true"></i>
                    </button>
                    <ul className="nested-list">
                      <li><Link to="/rooftop-residential-flats" onClick={closeMenu}>Residential Flat</Link></li>
                      <li><Link to="/rooftop-party-lounge" onClick={closeMenu}>Party Lounge</Link></li>
                      <li><Link to="/rooftop-guest-rooms" onClick={closeMenu}>Guest Room</Link></li>
                      <li><Link to="/rooftop-servant-rooms" onClick={closeMenu}>Servant Room</Link></li>
                    </ul>
                  </li>

                  {/* LGSF Modular Solutions */}
                  <li className={`nested-submenu${openNested === "lgsf" ? " mobile-nested-open" : ""}`}>
                    ...
                    <ul className="nested-list">
                      <li><Link to="/prefab-dry-wall" onClick={closeMenu}>Prefab Drywall</Link></li>
                      <li><Link to="/prefab-residential-house" onClick={closeMenu}>Prefab Residential House (G+1, G+2)</Link></li>
                      <li><Link to="/lgsf-luxury-lounges" onClick={closeMenu}>Luxury Lounge</Link></li>
                      <li><Link to="/lgsf-showroom" onClick={closeMenu}>Showrooms</Link></li>
                      <li><Link to="/prefab-sales-office" onClick={closeMenu}>Prefabricated Sales Office</Link></li>
                      <li><Link to="/prefab-waiting-area" onClick={closeMenu}>Prefabricated Waiting Area</Link></li>
                      <li><Link to="/farmhouse-cottage" onClick={closeMenu}>Farmhouses / Cottages</Link></li>
                    </ul>
                  </li>
              ...
              <li className={`dropdown-submenu${openSub === "solar" ? " mobile-sub-open" : ""}`}>
                ...
                <ul className="submenu-list">
                  ...
                  <li><Link to="/solar-mounting-structure" onClick={closeMenu}>Module Mounting Structure</Link></li>
                  <li><Link to="/hardware-solutions" onClick={closeMenu}>Hardware Solutions</Link></li>
                </ul>
              </li>

              {/* 4. HVAC SOLUTIONS — direct */}
              <li>
                <Link to="/hvac-solutions" onClick={closeMenu}>HVAC Solutions</Link>
              </li>
